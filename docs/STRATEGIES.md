...
| AMD / Power of 3: accumulation range → manipulation sweep → distribution | `overnight_range()` + `level_sweep()`: the "accumulation" range is the overnight/pre-market session (prior 16:00 ET close → today's 09:30 open, from extended-hours bars); a sweep of its high/low that closes back inside is the "manipulation" leg, and the trade is the "distribution" expansion (`SWEEP_OVERNIGHT_RANGE=true`). |
